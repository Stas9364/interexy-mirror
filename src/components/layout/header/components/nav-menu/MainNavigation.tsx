    ...
    <div className='hidden h-full items-center lg:flex'>
      {menuData.map(({ title, subtitle, submenu, link }) => (
        <div key={title} className='group relative h-full'>
          ...
          {!link && (
            <div className='invisible fixed top-[100px] right-0 left-0 z-50 opacity-0 shadow-lg transition-all duration-200 ease-in-out group-hover:visible group-hover:opacity-100'>
