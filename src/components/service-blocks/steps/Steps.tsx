      ...
      <Container>
        ...
        <ul className='flex grid-cols-3 flex-col gap-y-10 md:grid md:gap-6'>
          {steps.map(({ title, description, href, linkTitle, color }, idx) => (
            <li key={title} className='flex flex-col text-white'>
              <div>
                ...
                <h3 className='mb-[10px] text-xl leading-[1.4] font-semibold'>{title}</h3>
                <p className='mb-6 text-base leading-[1.4] font-normal'>{description}</p>
              </div>
              <Link
